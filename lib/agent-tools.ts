...
const writeGoalTool = tool(
  ...
  'Writes a goal file with correct frontmatter and path structure. Only use after the user has approved the suggested goals.',
  {
    ...
    startYear: z.number().optional().describe('Vision start year (required for vision period type)'),
    endYear: z.number().optional().describe('Vision end year (required for vision period type)'),
    emoji: z.string().optional().describe('Single emoji for yearly goals (e.g. "🚀")'),
    theme: z.string().optional().describe('Theme text for yearly goals'),
